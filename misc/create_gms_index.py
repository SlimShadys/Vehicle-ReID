...
if (dataset == 'veri_776'):
    data_path = os.path.join(base_datapath, 'VeRi-776')
    img_dir = os.path.join(data_path, 'image_train')
    train_file = os.path.join(data_path, 'train_label.xml')
elif (dataset == 'veri_wild'):
    data_path = os.path.join(base_datapath, 'VeRi-Wild')
    img_dir = os.path.join(data_path, 'images')
    train_file = os.path.join(
        data_path, 'train_test_split', 'train_list_start0.txt')
...
with tqdm(total=total_iterations, desc="Processing pickle files") as pbar:
    for class_label, images in class_images.items():
        ...
        adj_matrix = process_class(images, image_size, verbose)
