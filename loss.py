...
from losses.triplet_loss import TripletLoss, TripletLossV2
...
class LossBuilder(nn.Module):
    def __init__(self, loss_type="TripletLoss", alpha=0.9, k=10, margin=0.3, label_smoothing=0.1, apply_MALW=False, batch_size=20, use_amp=False) -> torch.nn.Module:
        ...
        self.loss_type = loss_type              # Loss type (TripletLoss, TripletLossV2, SupConLoss, SupConLoss_Pytorch, TripletMarginLoss_Pytorch)
        ...
        ## =========== Metric Loss =========== ##
        if loss_type == "TripletLoss": # Triplet Loss
            self.triplet_loss = TripletLoss(self.margin, self.use_amp)
        elif loss_type == "TripletLossV2":
            self.triplet_loss = TripletLossV2(self.margin, self.use_amp)     
        elif loss_type == "SupConLoss": # Supervised Contrastive Loss from SupConLoss
            self.triplet_loss = SupConLoss(num_ids=6, views=6, contrast_mode='all') 
        elif loss_type == "SupConLoss_Pytorch": # Supervised Contrastive Loss from Pytorch Metric Learning
        ...
        elif loss_type == "TripletMarginLoss_Pytorch": # Triplet Margin Loss from Pytorch Metric Learning
    ...
    def forward(self, embeddings: torch.Tensor, pred_ids: torch.Tensor, target_ids: torch.Tensor, normalize: bool = False, use_rptm: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        elif self.loss_type == "TripletLoss" or self.loss_type == "TripletLossV2":
            metric_loss, _, _ = self.triplet_loss(embeddings, target_ids)
        else:
            raise ValueError("Invalid loss type. Choose between 'TripletLoss', 'TripletLossV2', 'SupConLoss', 'SupConLoss_Pytorch' or 'TripletMarginLoss_Pytorch'")
